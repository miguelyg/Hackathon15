                                ...
                                <form @submit.prevent="editContact()">
                                    ...
                                    <div class="level">
                                        <div class="buttons level-item has-text-centered">
                                            <input type="submit" class="button is-primary" value="Editar">
...
export default {
    ...
    methods: {
        ...
        const id = `${this.$route.params.id}`;
            ...
            const data = await fetch(`http://localhost:3000/contacts/${id}`,{method:'PUT',body:JSON.stringify(body), headers: { 'Content-Type': 'application/json' }});
            const info = await data.json();
            this.goContacts();

        },
        async getContact(){
            const data = await fetch(`http://localhost:3000/contacts/${this.$route.params.id}`);
            const info = await data.json();
            this.contact = info;
        },
        goContacts(){
